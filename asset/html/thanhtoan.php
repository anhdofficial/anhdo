          ...
          <div class="thanhtoan__box">
               <div class="gird_row">
                    <form action="" method="post">
                         ...
                         <label for="so_luong_nguoi" class="thanhtoan-label">
                              Số lượng người
                              <select name="so_luong_nguoi" id="so_luong_nguoi" class="thanhtoan-btn" required>
                                   <?php
                                   // Bạn có thể điều chỉnh phạm vi của lựa chọn dựa trên yêu cầu của mình
                                   for ($i = 1; $i <= 4; $i++) {
                         ...
                         <script>
                              // Thêm sự kiện lắng nghe cho trường số lượng người
                              document.getElementById('so_luong_nguoi').addEventListener('change', function() {
                                   // Lấy số lượng người được chọn
                                   var selectedQuantity = this.value;

                                   // Lấy giá gốc (loại bỏ 'đ' và chuyển đổi thành số nguyên)
                                   var originalPrice = parseInt("<?php echo $giatour; ?>".replace('đ', '').trim());

                                   // Tính toán tổng giá dựa trên số lượng người
                                   var totalPrice = selectedQuantity * originalPrice;

                                   // Định dạng số với ký tự phân cách hàng nghìn
                                   var formattedTotalPrice = totalPrice.toLocaleString('en-US');

                                   // Hiển thị tổng giá trong trường nhập giá tour
                                   document.getElementById('giatour').value = formattedTotalPrice + 'đ';
                              });
                         </script>
                    </form>
                    <?php
                    ...
                    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                         ...
                         $so_luong_nguoi = mysqli_real_escape_string($conn, $_POST['so_luong_nguoi']);
                         ...
                         $query = "INSERT INTO hoadon (ma_san_pham, ten_san_pham, giatour, so_luong_nguoi, ho_ten, email, so_dien_thoai, dia_chi, phuong_thuc_thanh_toan, user_id) 
              VALUES ('$ma_san_pham', '$ten_san_pham', '$giatour', '$so_luong_nguoi', '$ho_ten', '$email', '$so_dien_thoai', '$dia_chi', '$phuong_thuc_thanh_toan', '$user_id')";
